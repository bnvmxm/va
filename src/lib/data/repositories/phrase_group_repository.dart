...
class PhraseGroupRepository {
  Map<int, String> findKnownNames() => <int, String>{}..addEntries(
      svc.dataProvider.dataGroups.map((x) => MapEntry(x.groupId, x.name)));

  Iterable<PhraseGroup> findMany() =>
      svc.dataProvider.dataGroups.map((x) => x.toModel());

  PhraseGroup? findSingle(int? groupId) {
    final dto = svc.dataProvider.dataGroups
        .firstWhereOrNull((x) => x.groupId == groupId);
    return dto?.toModel();
  }
  ...
  PhraseGroup create(String name) {
    final dto = DataGroup(
        groupId:
            _findMaxGroupId() + 1 /* Autoincrement in a real case of course */,
        name: name,
        phrases: {});
  ...
  PhraseGroup? rename(int id, String toName) {
    final dto =
        svc.dataProvider.dataGroups.firstWhereOrNull((x) => x.groupId == id);
